                    ...
                    <div class="form-group text-left">
                        ...
                        <div>
                            <input type="text" class="form-control" placeholder="Course Name"
                                name="courseName" value="{{ old('courseName') }}" />
                    ...
                    <div class="form-group text-left">
                        ...
                        <div>
                            <input type="text" class="form-control" placeholder="Subject Name"
                                name="courseYearLevel" value="{{ old('courseYearLevel') }}" />
                            <small class="text-danger" id="yearLevel-error"></small>
                        </div>
                    </div>
                    <div class="form-group text-left">
                        <label>Section</label>
                        <div>
                            <textarea name="courseSection" id="" cols="30" rows="3" class="form-control"
                                placeholder="Section"></textarea>
                            <small class="text-danger" id="section-error"></small>
                        </div>
                    </div>
                    <hr class="hr-divider">
...
@section('scripts')
    <script>
        ...
        {{\Route::currentRouteName()}}Form.addEventListener('submit', async (e) => {
            ...
            let courseNameError = document.querySelector('#courseName-error');
            let yearLevelError = document.querySelector('#yearLevel-error');
            ...
            await axios.post('/dashboard/store-{{\Route::currentRouteName()}}', formData)
                ...
                .catch(function(error) {
                    error.response.data.errors.courseName ?
                        courseNameError.innerHTML = error.response.data.errors.courseName :
                        courseNameError.innerHTML = '';

                    error.response.data.errors.courseYearLevel ?
                        yearLevelError.innerHTML = error.response.data.errors.courseYearLevel :
                        yearLevelError.innerHTML = '';

                    error.response.data.errors.courseSection ?
                        sectionError.innerHTML = error.response.data.errors.courseSection :
                        sectionError.innerHTML = '';
                })
        })
    </script>
